...
describe('SyncEngine', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    // Reset singleton
    (SyncEngine as any).instance = null;
  ...
  describe('Queue Operations', () => {
    ...
    it('should queue operations with all required fields', async () => {
      ...
      // Check each call
      for (let i = 0; i < 3; i++) {
        const callArgs = (mockDb.insert as jest.Mock).mock.calls[i][1];
        expect(callArgs).toHaveProperty('id');
        expect(callArgs).toHaveProperty('operation');
        expect(callArgs).toHaveProperty('entityType');
  ...
  describe('Offline Data Availability', () => {
    ...
    it('should cleanup old data', async () => {
      ...
      await (syncEngine as any).cleanupOldData();
  ...
  describe('Sync Initialization', () => {
    it('should initialize sync engine', async () => {
      (mockDb.initialize as jest.Mock).mockResolvedValue(undefined);
  ...
  describe('Sync Shutdown', () => {
    it('should shutdown sync engine', async () => {
      (mockDb.initialize as jest.Mock).mockResolvedValue(undefined);
      (mockDb.close as jest.Mock).mockResolvedValue(undefined);

      await syncEngine.initialize();
      await (syncEngine as any).shutdown();
    ...
    it('should stop periodic sync on shutdown', async () => {
      ...
      await syncEngine.initialize();
      (syncEngine as any).stopPeriodicSync();
